...
def generate_gradcam(model, input_tensor, target_class=None):
    ...
    target_layer = get_target_layers(model_name, model)[0]

    # 创建 Grad-CAM 提取器
    cam_extractor = GradCAM(model=model.net, target_layers=[target_layer])

    # 生成 CAM
    cam = cam_extractor(input_tensor=input_tensor, targets=target_class)
    
    # 获取预测类别
    with torch.no_grad():
        scores = model.net(input_tensor)
        if target_class is None:
            predicted_class = scores.argmax(dim=1).item()
        else:
            predicted_class = target_class

    # cam 是 numpy array, shape [batch_size, H, W]
    cam_map = cam[0]  # 取第一个样本的 CAM
    return cam_map, predicted_class
def visualize_and_save_gradcam(image_path, model, model_name, save_dir, device):
    ...
    # 使用 pytorch-grad-cam 提供的工具函数生成可视化
    visualization = show_cam_on_image(original_image_np, cam, use_rgb=True)
...
def main(args):
    ...
    save_dir = args.save_dir or "visualizations/gradcam"
...
if __name__ == "__main__":
    ...
    parser.add_argument("--save_dir", type=str, default=None, help="Directory to save the visualizations")
